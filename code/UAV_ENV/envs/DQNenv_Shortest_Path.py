...
class DQNUAVenv_SD(gym.Env):
    ...
    def __init__ (self):
        ...
        self.SINR_threshold=10                        # SINR threshold in dB 
        ...
        self.uav_power= np.array([14,16,18,20])
        ...
        self.x_uav_loc=np.arange(self.Min_x,self.uav_initial_loc[0]+1)   # initial x is larger that terminal x
    ...
    def reward_fun(self,info,state,next_uav_loc):
        ...
        # distance from current position to terminal location
        ...
        # check the condition
            ...
            reward=1/(dis/self.total_distance)
            done=False
            if  all(x >= self.SINR_threshold for x in SINR):
                ...
                #reward=math.log10((Rate/self.sum_distance)+(self.total_distance/dis))
            ...
            if  all(x >= self.SINR_threshold for x in SINR):
                ...
                #reward=math.log10(Rate/self.sum_distance)
                #print(f"reward is:{reward}")
    ...
    def path_plot(self,x_array,y_array,z_array):
        ...
        ax.view_init(20, 8)
        #plt.show()
        plt.savefig(r'C:\Users\student\OneDrive - Carleton University (1)\Erricson\Third paper\Simulation_DRL\Plots\Trajectory_SD.pdf')

        fig2 = plt.figure()
        ax = plt.axes(projection='3d')
        plt.plot(self.GUE1_loc[:,0],self.GUE1_loc[:,1],self.GUE1_loc[:,2],c='blue')
        #plt.savefig(r'C:\Users\student\OneDrive - Carleton University (1)y\Erricson\Third paper\Simulation_DRL\Plots\U1.pdf')

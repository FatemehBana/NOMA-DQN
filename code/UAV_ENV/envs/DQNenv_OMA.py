...
class DQNUAVenv_OMA(gym.Env):
    
    def __init__ (self):
        
        # simulation parameters
        self.Num_GUE=2
        self.Bandwidth=50e6                          # 20MHz       
        ...
        #self.SINR_threshold=1                        # SINR threshold in dB 
        ...
        self.Data_Rate={"Rate_uav":0, "Distance":0, "Rate_OMA":0}
        self.sum_distance=0
        self.seed()
        ...
        self.user=self.Num_GUE+1
        
        self.action_space=Discrete(len(self.uav_move))
        ...
        # considering hieght 1m for two ground users
        self.GUE1_loc[:,2]=1
        self.GUE2_loc[:,2]=1
    ...
    def step(self,action):
        ...
        direction_uav=self.uav_move[action]
        ...
        # sum rate of two other users in RB 1 and 2
        SINR_GUE1=((self.GUE_power-abs(self.channel_gue1[index_GUE1[0]+1][0]))-
                      (self.N_0+(10*math.log10(self.Bandwidth/self.user))))
        
        SINR_GUE2=((self.GUE_power-abs(self.channel_gue2[index_GUE2[0]+1][0]))-
                      (self.N_0+(10*math.log10(self.Bandwidth/self.user))))
        
        Rate_GUE1=((self.Bandwidth/self.user)*math.log2(1+(10**(SINR_GUE1/10))))/1e6
        Rate_GUE2=((self.Bandwidth/self.user)*math.log2(1+(10**(SINR_GUE2/10))))/1e6
        Sum_Rate_OMA=Rate_GUE1+Rate_GUE2
        ...
        Rate,self.reward,self.done=self.reward_fun(Ch_uav,state,next_uav_loc)
        self.Data_Rate.update({"Rate_uav":Rate, "Distance":self.sum_distance, "Rate_OMA":Sum_Rate_OMA})
    ...
    def reset(self,seed=None,options=None):
        ...
        self.state=np.array(np.concatenate((self.uav_initial_loc,self.GUE1_loc[0],self.GUE2_loc[0])))
        self.done=False
        self.Data_Rate.update({"Rate_uav":0, "Distance":0, "Rate_OMA":0})
        self.sum_distance=0
        #self.sum_energy=0
        #self.energy_usage=0
        self.reward=0
    ...
    def reward_fun(self,Ch_uav,state,next_uav_loc):
        ...
        SINR_uav=((self.UAV_max_power-abs(Ch_uav))-
                      (self.N_0+(10*math.log10(self.Bandwidth/self.user))))
        ...
        # check the condition
            ...
            Rate=((self.Bandwidth/self.user)*(math.log2(1+(10**(SINR_uav/10)))) /1e6)
            #print(f"Rate is:{Rate}")
            #reward=(Rate)/(self.sum_distance+dis)
            ...
            Rate=((self.Bandwidth/self.user)*(math.log2(1+(10**(SINR_uav/10)))) /1e6)
            #print(f"Rate is:{Rate}")
            #reward=math.log10((Rate/self.sum_energy))
    ...
    def path_plot(self,x_array,y_array,z_array):
        ...
        ax.view_init(20, 8)
        #plt.show()
        plt.savefig(r'C:\Users\student\OneDrive - Carleton University (1)\Erricson\Third paper\Simulation_DRL\Plots\Trajectory_OMA.pdf')
